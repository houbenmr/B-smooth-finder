 ...
 *                 (default: empirical prime-valued function of log(N))
 ...
 *   ./find_smooth_path p B N phi_directory [options]
 *   ./find_smooth_path p j1_re j1_im B N phi_directory [options]
 *   ./find_smooth_path p j1_re j1_im j2_re j2_im B N \
 ...
 *   - The two degree-bounded balls are enumerated by interleaved Dijkstra
 *     searches. The side with the smaller pending multiplicative degree is
 *     advanced next, alternating ties. After every evaluation batch, newly
 *     discovered vertices are checked against the other ball, so the
 *     search returns as soon as a common j-invariant is discovered.
 *   - Equal-distance Dijkstra vertices are processed together. For batches
 *     of at least MULTIPOINT_THRESHOLD points, coefficient polynomials are
 *     evaluated with FLINT's precomputed multipoint tree.
 *   - Smaller batches use Horner evaluation, since a multipoint tree costs
 ...
 *   - Dijkstra ordering is required because degrees multiply by ell; BFS
 *     would minimize the number of steps rather than the isogeny degree.
 *   - The priority queue is indexed and supports decrease-key, so it never
 *     stores stale duplicate entries.
    ...
    size_t *this_meet, size_t *other_meet)
{
    if (field->kind == BACKEND_FQ_NMOD)
        return process_frontier_batch_word(
            ...
            stats, other, this_meet, other_meet);
...
typedef struct {
    double upper_log_N;
    unsigned B;
} default_B_interval_t;

/*
 * Empirical prime-valued step function fitted to the supplied
 * (log(N), optimal B) samples. Each boundary is the midpoint between the
 * last sample for one prime and the first sample for the next prime.
 */
static unsigned default_bound_from_N(const fmpz_t N)
{
    static const default_B_interval_t intervals[] = {
        {2.62874768601389, 2},
        {4.49857357575757, 3},
        {6.12233889232062, 5},
        {7.50894104046162, 7},
        {8.66420157434004, 11},
        {9.588497953445275, 13},
        {10.51273111262535, 17},
        {11.2058782931853, 19},
        {11.89902547374525, 23},
        {12.5921726543052, 29},
        {13.05427110587645, 31},
        {13.5163694580933, 37},
        {13.9784677723580, 41},
        {14.4405660677741, 43},
        {14.90266415864055, 47},
        {15.36476241732745, 53},
        {15.8268605790883, 59},
        {16.05790964519565, 61},
        {16.52000779719685, 67},
        {16.75105684940975, 71},
        {16.98210590344785, 73},
        {17.4442040417967, 79},
        {17.67525309583485, 83},
        {17.90630215831675, 89}
    };
    double log_N = fmpz_dlog(N);
    size_t i;

    for (i = 0; i < sizeof(intervals) / sizeof(intervals[0]); i++)
        if (log_N < intervals[i].upper_log_N)
            return intervals[i].B;
    return 97;
}
...
static unsigned parse_bound(const char *s)
{
    ...
    if (errno || *end != '\0' || value < 2 || value > 100)
        die("B must satisfy 2 <= B <= 100");
...
{
    jkey_t j1, target, rerandomized_start, rerandomized_target;
    ...
    key_init(&j1);
    key_init(&target);
    key_init(&rerandomized_start);
    key_init(&rerandomized_target);
    if (j_mode >= 1)
    ...
    if (allow_rerandomization)
        fprintf(stderr,
                ...
                "(ell' = %u)%s\n",
                B, B_defaulted ? " [default from log(N)]" : "",
                rerandomization_ell,
    ...
    found = search_between(&left, &right, j1, target, N,
    ...
    if (found) {
        ...
        for (;;) {
            ...
            for (rerandomization_index = 0;
                 rerandomization_index < current_length;
                 rerandomization_index++) {
                successful_endpoint =
                    current_layer[rerandomization_index];
                key_set(
                    &rerandomized_start,
                    &rerandomization_tree.nodes[successful_endpoint].key);
                if (j_mode == 2)
                    key_set(&rerandomized_target, &target);
                else
                    frobenius_conjugate(&rerandomized_target,
                                        &rerandomized_start);
                rerandomizations_attempted++;
                fprintf(stderr,
                        "\nRerandomization attempt %lu "
                        "(depth %lu, walk %lu/%lu)\n"
                        "  start j' = ",
                        ...
                        (unsigned long)current_length);
                fprint_key(stderr, rerandomized_start);
                fprintf(stderr, "\n  target %s = ",
                        j_mode == 2 ? "j2" : "(j')^p");
                fprint_key(stderr, rerandomized_target);
                fprintf(stderr, "\n");

                found = search_between(
                    &left, &right, rerandomized_start,
                    rerandomized_target, N, polynomials, n_polynomials,
                    &field, n_threads, "j'",
                    j_mode == 2 ? "j2" : "(j')^p", &left_meet,
                    &right_meet, path_degree);
                graphs_live = 1;
                attempted_rerandomization = 1;
                if (found)
                    break;

                fprintf(stderr, "Rerandomization attempt %lu failed\n",
                        (unsigned long)rerandomizations_attempted);
                graph_clear(&left);
                graph_clear(&right);
                graphs_live = 0;
    ...
    free(polynomials);
    key_clear(&rerandomized_target);
    key_clear(&rerandomized_start);
    key_clear(&target);
    key_clear(&j1);
    ...
    printf("\nTiming (wall clock, %u worker thread%s)\n", n_threads,
           n_threads == 1 ? "" : "s");
    ...
    if (attempted_rerandomization)
        printf("  rerandomized searches:  %.3f s (%lu attempt%s)\n",
